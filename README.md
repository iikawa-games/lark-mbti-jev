...
在控制窗口的“标签显示”或托盘右键菜单的“标签显示”中，可以把 MBTI 换成动漫角色，例如 **鹿目圆 65%**。百分比不变，仍是模型对该 MBTI 类型给出的概率。双击成员可以看到对应的角色、作品和 MBTI 类型。
...
**为某部动漫自定义主题**：点击“自定义主题…”，程序会打开本机的 `.data/themes` 文件夹，里面有一个示例文件。复制这个文件并改名（文件名不要以 `_` 开头），为 16 种类型填写角色，保存后即可在“标签显示”中选择。没有填写的类型继续显示 MBTI。文件格式如下：
